...
describe('InstitutionalProfileService', () => {
  ...
  const service = new InstitutionalProfileService(
  ...
  it('reads only report-safe fields from the supplied transaction client', async () => {
    const reportProfile = {
      legalName: null,
      legalIdentification: null,
      dinadecoRegistrationCode: null,
  ...
  it('audits a no-op patch without creating another organization', async () => {
    ...
    expect(audit.log).toHaveBeenCalledWith(
      expect.objectContaining({ details: { changedFields: [] } }),
      tx,
    );

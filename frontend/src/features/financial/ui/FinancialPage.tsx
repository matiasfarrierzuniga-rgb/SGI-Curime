  ...
  return <section className="space-y-6">
    ...
    {list.data && list.data.data.length > 0 ? <>
      <div className="overflow-x-auto rounded-xl border border-border" tabIndex={0} aria-label="Listado de cargos financieros">
        <table className="min-w-[900px] w-full text-left text-sm"><thead className="bg-surface-muted text-foreground-muted"><tr><th className="p-3">Cargo</th><th className="p-3">Reserva</th><th className="p-3">Monto</th><th className="p-3">Saldo</th><th className="p-3">Moneda</th><th className="p-3">Estado</th><th className="p-3">Creado</th><th className="p-3">Acciones</th></tr></thead><tbody>{list.data.data.map(charge => <tr key={charge.id} className="border-t border-border align-top"><td className="p-3 font-semibold">#{charge.id}</td><td className="p-3">#{charge.reservationId}</td><td className="p-3 whitespace-nowrap">{formatFinancialCurrency(charge.amount, charge.currency)}</td><td className="p-3 whitespace-nowrap">{formatFinancialCurrency(charge.balance, charge.currency)}</td><td className="p-3">{charge.currency}</td><td className="p-3"><StatusBadge variant={financialChargeStatusVariant(charge.status)}>{financialChargeStatusLabel(charge.status)}</StatusBadge></td><td className="p-3 whitespace-nowrap">{formatFinancialDate(charge.createdAt)}</td><td className="p-3"><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" type="button" onClick={() => setSelectedId(charge.id)}>Ver detalle</Button>{charge.status === 'PENDING' && hasCapability(user?.role, 'fin.payments.record') ? <Button size="sm" type="button" onClick={() => setPayingCharge(charge)}>Registrar pago completo</Button> : null}</div></td></tr>)}</tbody></table>
      </div>

...
import { useAffiliateDetail } from '../hooks/useAffiliatesQueries'
...
export function AffiliateDetailsModal({ id, onClose, onEdit }: AffiliateDetailsModalProps) {
  ...
  return (
    <Modal title="Detalle de afiliado" onClose={onClose} busy={detailQuery.isFetching}>
      {detailQuery.isPending ? (
        ...
        <>
          ...
          <div className="actions"><button type="button" onClick={() => onEdit(detailQuery.data)}>Editar datos</button></div>
        </>
      ) : null}
    </Modal>
  )

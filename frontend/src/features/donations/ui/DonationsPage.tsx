...
function DonationFilters({ filters, onChange, onClear, hasActiveFilters }: { filters: DonationListFilters; onChange: (change: Partial<DonationListFilters>) => void; onClear: () => void; hasActiveFilters: boolean }) {
  return <form className="grid gap-4 rounded-xl border border-border bg-surface p-4 sm:grid-cols-2 lg:grid-cols-3" onSubmit={event => event.preventDefault()}><Label className="grid gap-1" htmlFor="donation-search">Buscar<Input id="donation-search" value={filters.search ?? ''} placeholder="Nombre, identificación o referencia" onChange={event => onChange({ search: event.target.value || undefined })} /></Label><Label className="grid gap-1" htmlFor="donation-status">Estado<Select id="donation-status" value={filters.status ?? ''} onChange={event => onChange({ status: event.target.value ? event.target.value as DonationStatus : undefined })}><option value="">Todos</option><option value="CONFIRMED">Confirmada</option><option value="CANCELLED">Cancelada</option></Select></Label><Label className="grid gap-1" htmlFor="donation-method-filter">Método<Select id="donation-method-filter" value={filters.method ?? ''} onChange={event => onChange({ method: event.target.value ? event.target.value as DonationMethod : undefined })}><option value="">Todos</option>{donationMethods.map(method => <option key={method} value={method}>{donationMethodLabel(method)}</option>)}</Select></Label><Label className="grid gap-1" htmlFor="donation-date-from">Desde<Input id="donation-date-from" type="date" value={filters.dateFrom ?? ''} onChange={event => onChange({ dateFrom: event.target.value || undefined })} /></Label><Label className="grid gap-1" htmlFor="donation-date-to">Hasta<Input id="donation-date-to" type="date" value={filters.dateTo ?? ''} onChange={event => onChange({ dateTo: event.target.value || undefined })} /></Label><div className="flex items-end"><Button type="button" variant="outline" disabled={!hasActiveFilters} onClick={onClear}>Limpiar filtros</Button></div></form>
}
